...
<div class="col s12 m8" style="position: relative;top:8px;">
  ...
  <table>
    ...
    <tbody style="background: #eff2f5;">
      <?php
  $noticias = mysqli_query($conn, "SELECT * FROM messages_privates WHERE author = '".$user['username']."'");
    while ($noticiast = mysqli_fetch_assoc($noticias)) {
                  ?>
      <tr>
        <td><?php echo $noticiast['title'];?></td>
        <td><?php echo $noticiast['receptor'];?></td>
        <td><?php echo $noticiast['data'];?></td>
        <td>
         
          <a href="<?php echo $site;?>/smsenv.php?action=err&id=<?php echo $noticiast['id'];?>">Supprimer</a>
        </td>
      </tr>
    <?php } ?>
    </tbody>
