...
<div class="col s12 m8" style="position: relative;top:8px;">
  <?php
  $news = mysqli_query($conn, "SELECT * FROM ranks WHERE id in (3) ORDER BY id DESC");
 ...
 <div class="habbostaff">
   <img src="https://habbo.es/habbo-imaging/avatarimage?figure=<?php echo $staffsuv['look']; ?>&action=std&gesture=srp&direction=2&head_direction=3&size=l&headonly=1&img_format=png" style="float: left;position: relative;top:-15px;"><b><?php
              if ($staffsuv['online'] == 1) {
                echo "<span class='staff_online'></span>";
              } else {
 ...
 <div class="infostaff">
  <b><?php
            ...
            ?><a href="<?php echo $site;?>/profil/<?php echo $staffsuv['id']; ?>" style="color:#FFF;"><?php echo $staffsuv['username']; ?></a></b><br>
      <?php
...
<?php } ?>
    ...
    <div class="col s12 m4" style="position: relative;top:8px;">
       <div class="title-blue">Equipe des jeux</div>
      <div class="card">
        <div class="infohotel">
<center>Les <?php
$resultado = mysqli_query($conn,"SELECT * FROM users WHERE rank in (3)' AND online = '1'");
$número_filas = mysqli_num_rows($resultado);
 
echo "$número_filas";
 
?> <b>Responsable</b> des jeux.</center>
</div>
